...
return cloneref(game:GetService("CoreGui")), end, Library.Directory = "Zolar", Library.ConfigFolder = "Zolar/Configs", Library.AssetsFolder = "Zolar/Assets", if
 isfolder then
 for _, Folder in { Library.Directory, Library.ConfigFolder, Library.AssetsFolder } do
 if not isfolder(Folder) then
makefolder(Folder), end, end, end, local UiFont, local UiFontBold
 local CustomFont = { } do
 local FontMagics = {
"\0\1\0\0",, "OTTO",, "true",, "ttcf",, "wOFF", }, local function LooksLikeFont(Body)
...
writefile(FontPath, Body)
 ...
 local FontData = {
name = Name,, faces = {, {, name = "Regular",, weight = Weight,, style = Style,, assetId = getcustomasset(FontPath), }, }, }, writefile(JsonPath, 
...
UiFontBold = UiFont, end, Library.Font = UiFont, Library.TitleFont = UiFontBold, local IconPack, pcall(function()
...
return Result, end, end, return "rbxassetid://0", end, local function ToVector2(Value)
...
end, Library.__index = Library, Library.Version = "1.1", Library.WindowWidth = 716, Library.WindowHeight = 540, Library.Theme = {
...
}, Library.AccentPresets = {, Color3.fromRGB(179, 165, 255),, Color3.fromRGB(120, 132, 255),, Color3.fromRGB(96, 170, 255),, Color3.fromRGB(72, 214, 168),, 
...
Preset[Key] = Value, end, return Preset, end, Library.ThemePresets = {, MakePreset("Default", Library.Theme),, MakePreset("Azure", {
...
}), }, Library.ThemeKeys = {, "Background",, "Section",, "Element",, "Light",, "Line",, "Text",, "DimText", }, local function DeriveTheme()
...
DeriveTheme()
 ...
 Library.Searchables = { }
Library.MenuKeybind = Enum.KeyCode.G, Library.Binding = false, Library.UserScale = 1, Library.Silent = false, Library.ThemeDirty = false, Library.PreloadDirty =
 false, Library.PreloadClock = 0
 Library.Preloaded = setmetatable({ }, { __mode = "k" })
Library.Animation = {, Time = 0.25,, Style = Enum.EasingStyle.Quart,, Direction = Enum.EasingDirection.Out, }
...
Data.Instance.Name = "\0", continue, end, Data.Instance[Property] = Value, end
 if Class == "ImageLabel" or Class == "ImageButton" then
Library.PreloadDirty = true, end, if Library.SeedBaseline then, Library:SeedBaseline(Data.Instance), end, return setmetatable(Data, Library), end, 
...
Library:Thread(function(), pcall(function(), ContentProvider:PreloadAsync(Assets), end), end), end
...
LastFire = Now, Callback(Input), end, table.insert(Library.TouchButtons, {, Instance = Self.Instance,, Fire = Fire, }), Connection = 
Self.Instance.Activated:Connect(function(Input), Fire(Input), end), else, Connection = Self.Instance[Signal]:Connect(Callback), end, else, Connection = 
Signal:Connect(Callback), end, table.insert(Library.Connections, Connection), return Connection, end
...
Handler(Child, Property), end, end, end, local function RestoreResting(Children), ForEachFadeable(Children, function(Child, Property)
...
Library.RestingValues[Object] = Store
 ...
 if Known == nil then
Library:SetBaseline(Object, Property, Store[Property]), end, end, return Store[Property], end
...
return Value
 ...
 if not Store then
 Store = { }
Library.RestingValues[Object] = Store, end, Store[Property] = Value, Library:SetBaseline(Object, Property, Value), end, Library.HardRestore = function(Self)
...
pcall(function(), Child[Property] = Base[Property], end), end, end), end
...
Object[Property] = 1
 ...
 if not Ok then
pcall(function(), Object[Property] = Target, end), end, end
...
RestoreResting(Children)
 ...
 end), end, Library.CancelFade = function(Self), BumpFadeToken(Self.Instance), end, Library.ResetFade = function(Self)
 local Root = Self.Instance
BumpFadeToken(Root)
...
Object[Property] = Library.Theme[Value], else, Object[Property] = Value(), end, end, table.insert(Library.ThemingStuff, ThemeData), Library.ThemeMap[Object] = 
ThemeData, return Self, end
 Library.ChangeItemTheme = function(Self, Properties)
 local Object = Self.Instance
 if not Library.ThemeMap[Object] then return end
Library.ThemeMap[Object].Properties = Properties, end, local function AccentSequence(), return ColorSequence.new({, ColorSequenceKeypoint.new(0, 
Library.Theme.Accent),, ColorSequenceKeypoint.new(1, Library.Theme.AccentDark), }), end
...
pcall(function(), Shadow.Color = Library.Theme.Accent, end), end, end
...
Preset = Entry, break, end, end, end
 if type(Preset) ~= "table" then return end
 for Key, Value in Preset do
 if Key ~= "Name" and Key ~= "Swatch" and typeof(Value) == "Color3" then
Library.Theme[Key] = Value, end, end, DeriveTheme(), Library.ThemeDirty = true, end
...
return Library.UIScale.Instance.Scale, end, return 1, end, local function IsOverObject(Object)
...
end, Library.IsMouseOverFrame = function(Self), return IsOverObject(Self.Instance), end, local function IsOverAnyPopup()
...
Dragging = false, InputChanged:Disconnect(), InputChanged = nil, end, end), end), Library:Connect(UserInputService.InputChanged, function(Input)
 local IsMove = Input.UserInputType == Enum.UserInputType.MouseMovement
 local IsTouch = Input.UserInputType == Enum.UserInputType.Touch
 if (IsMove or IsTouch) and Dragging then
Set(Input), end, end), end, Library.Unload = function(Self)
...
Parent = GetHui(),, Name = "",, ZIndexBehavior = Enum.ZIndexBehavior.Global,, ResetOnSpawn = false,, IgnoreGuiInset = true,, DisplayOrder = 1000, })
 Library.PopupHolder = Library:Create("ScreenGui", {
Parent = GetHui(),, Name = "",, ZIndexBehavior = Enum.ZIndexBehavior.Global,, ResetOnSpawn = false,, IgnoreGuiInset = true,, DisplayOrder = 1001, })
 Library.UnusedHolder = Library:Create("ScreenGui", {
Parent = GetHui(),, Name = "",, Enabled = false,, ResetOnSpawn = false, }), do
...
BorderSizePixel = 0, }), task.defer(function(), GuiInset = -Probe.Instance.AbsolutePosition.Y, Probe.Instance:Destroy(), end), end
...
end, if Params.Round then, Corner(Frame.Instance, Params.Round), end, return Frame, end, local function MakeText(Params), return Library:Create("TextLabel", {, 
Parent = Params.Parent,, Name = "",
...
BorderSizePixel = 0
 ...
 end, ApplyIcon(Image.Instance, Params.Icon), return Image, end, local function MakeButton(Params), return Library:Create("TextButton", {, Parent = 
Params.Parent,, Name = "\0",, Text = "",, AutoButtonColor = false,, BackgroundTransparency = 1,
 Position = Params.Pos or UDim2.fromOffset(0, 0),
...
Parent = Params.Parent,, Name = "\0",, FontFace = UiFont,, TextColor3 = Library.Theme.Text,, PlaceholderColor3 = Library.Theme.DimText,
...
BorderSizePixel = 0, }):AddToTheme({, TextColor3 = "Text",, PlaceholderColor3 = "DimText", }), if IsMobile then
 local Focus = MakeButton({
Parent = Params.Parent,
Pos = Params.Pos,
 Size = Params.Size or UDim2.new(1, 0, 1, 0),
 Z = (Params.Z or 6) + 2
 }), Focus:Connect("MouseButton1Down", function(), Input.Instance:CaptureFocus(), end), end, return Input, end
...
S.Name = "\0", S.Color = Color, S.Spread = Spread, S.BlurRadius = Blur, S.Transparency = Transparency, S.Parent = Parent, return S, end)
 if Ok and Shadow then
Library:SetBaseline(Shadow, "Transparency", Transparency), return Shadow, end, return nil, end
...
if Shadow then, table.insert(Library.AccentShadows, Shadow), end, return Shadow, end, Library.DimCount = 0
 Library.Dims = { }
 for Index = 1, 3 do
...
BorderSizePixel = 0, }), Corner(Dim.Instance, 10), Library.Dims[Index] = Dim, end, Library.Dim = Library.Dims[1]
...
Library:StampResting(Dim.Instance, "BackgroundTransparency", Target), if Shown then, Dim.Instance.Parent = Hosts[Index], Dim.Instance.Visible = true, end
...
Dim.Instance.Visible = false, Dim.Instance.Parent = Library.UnusedHolder.Instance, end, end), end, Library.CloseAllPopups = function(Self)
...
Centers[Index] = Vector2.new(
 ...
 ), end, Library.UIScale.Instance.Scale = Scale, Library.PopupScale.Instance.Scale = Scale, if IsMobile then
...
Library.UserScale = Multiplier, Library:CloseAllPopups(), UpdateScale(), end, UpdateScale(), 
Library:Connect(workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"), function(), task.wait(), UpdateScale(), end)
...
Library:Connect(UserInputService.InputEnded, function(Input)
 ...
 if math.abs(Delta.X) > 25 or math.abs(Delta.Y) > 25 then return end
...
Watcher:Disconnect(), Watcher = nil, end), end), Library:Connect(UserInputService.InputChanged, function(Input)
 local IsMove = Input.UserInputType == Enum.UserInputType.MouseMovement
 local IsTouch = Input.UserInputType == Enum.UserInputType.Touch
 if (IsMove or IsTouch) and Active then
Handlers.OnMove(Input), end, end), end, local function PlaceBelow(GetAnchor), return function(Extra)
...
Frame.Instance.Parent = Library.PopupHolder.Instance, Frame.Instance.Position = Place(From), Frame.Instance.Visible = true, Library.TouchShields[Frame.Instance]
 = Level, Library:SetDim(true)
 Frame:Tween({ Position = Place(To) })
 for _, Value in Library.OpenFrames do
 if not KeepOpen(Value) then
Value:SetOpen(false), end, end, Library.OpenFrames[Popup] = Popup, Frame:FadeDescendants(true, function(), Popup.Debounce = false, end), else
...
Library.TouchShields[Frame.Instance] = nil, Frame.Instance.Parent = Library.UnusedHolder.Instance, end), end, end, Library:Connect(UserInputService.InputBegan, 
function(Input)
 local IsClick = Input.UserInputType == Enum.UserInputType.MouseButton1
 local IsTouch = Input.UserInputType == Enum.UserInputType.Touch
 if not IsClick and not IsTouch then return end
...
Popup:SetOpen(false), end), return Popup, end, local function MakeAccentRow(Params)
 local Z = Params.Z
 local Row = MakeFrame({
Parent = Params.Parent,
Pos = Params.Pos,
...
Line.Instance,, UDim2.fromOffset(3, 15),, UDim.new(0, 10),, 0, ), if Shadow then, SetRest(Shadow, "Transparency", 1), end
 ...
 end, end, return {, Row = Row,, Line = Line,, Label = Label,, Hit = Hit,, Shadow = Shadow,, SetActive = SetActive, }, end
 ...
 Color = "Element",, Round = 6,, Clip = true,, Z = Level, }), Items.Frame.Instance.Visible = false, Items.SearchHolder = MakeFrame({, Parent = 
 Items.Frame.Instance,
  Size = UDim2.new(1, 0, 0, SearchHeight),
 Color = "Element",
  Z = Level + 5
 }), Items.SearchHolder.Instance.Visible = false, MakeImage({, Parent = Items.SearchHolder.Instance,, Icon = "search",
 ...
 Parent = Items.Frame.Instance,, Name = "\0",, BackgroundTransparency = 1,, ScrollBarThickness = 0,, ScrollBarImageTransparency = 1,, Selectable = false,, Active
  = true,, AutomaticCanvasSize = Enum.AutomaticSize.Y,
  CanvasSize = UDim2.fromOffset(0, 0),
  Size = UDim2.new(1, 0, 1, 0),
  ZIndex = Level + 4,
 BorderSizePixel = 0, }), Library:Create("UIListLayout", {, Parent = Items.Scroll.Instance,, SortOrder = Enum.SortOrder.LayoutOrder,
 ...
 Data.Row.Instance.Visible = Match, end, end, Library:Connect(Items.Search.Instance:GetPropertyChangedSignal("Text"), function(), 
 ApplySearch(Items.Search.Instance.Text), end), function Popup:AddRow(Text)
 ...
 TextActiveX = 20,
  ...
  local Data = {
 Name = Text,, Selected = false,, Row = Built.Row, }, function Data:Set(Active, Instant), Data.Selected = Active, Built.SetActive(Active, Instant), end, 
 ...
 end), Built.Hit:Connect("MouseButton1Down", function(), Popup.OnPick(Data), end), table.insert(Popup.Order, Data), return Data, end, function Popup:Clear()
  for _, Data in Popup.Order do
 Data.Row.Instance:Destroy()
 end
  Popup.Order = { }
 end, return AttachPopup({, Popup = Popup,, Frame = Items.Frame,, Level = Level,, GetAnchor = GetAnchor,, Place = PlaceBelow(GetAnchor),, OnOpen = function()
  local Anchor = GetAnchor()
  local Scale = Library:GetScreenScale()
 ...
 end, end,, OnClose = function(), Items.Search.Instance.Text = "", end, }), end
 ...
 Round = 20,, Z = Base, }), Swatch.Halo.Instance.BackgroundColor3 = Default, SetRest(Swatch.Halo.Instance, "BackgroundTransparency", 0.72), Swatch.Core = 
 MakeFrame({, Parent = Swatch.Halo.Instance,
 ...
 Round = 20,
  Z = Base + 1
 }), Swatch.Shadow = MakeShadow(, Swatch.Core.Instance,, Default,, UDim2.fromOffset(0, 0),, UDim.new(0, 6),, 0.35, ), Swatch.Hit = MakeButton({, Parent = 
 Swatch.Halo.Instance,
  Z = Base + 2
 })
 ...
 Swatch.Core:Tween({, BackgroundColor3 = Color,, BackgroundTransparency = Alpha, }), if Swatch.Shadow then, pcall(function(), Swatch.Shadow.Color = Color, end), 
 end, end, return Swatch, end
 ...
 Color = "Section",, Round = 8,, Z = Level, }), Items.Window.Instance.Visible = false
 ...
 AutoButtonColor = false,
  ...
  ZIndex = Level + 1,
 BorderSizePixel = 0, }), Corner(Items.Field.Instance, 8), Items.Tint = MakeFrame({, Parent = Items.Field.Instance,
  Size = UDim2.new(1, 0, 1, 0),
  Raw = Color3.new(1, 1, 1),
 Round = 8,
  Z = Level + 2
 }), Library:Create("UIGradient", {, Parent = Items.Tint.Instance,, Transparency = NumberSequence.new({, NumberSequenceKeypoint.new(0, 0),, 
 NumberSequenceKeypoint.new(1, 1), }), }), Items.Shade = MakeFrame({, Parent = Items.Field.Instance,
  Size = UDim2.new(1, 0, 1, 0),
  Raw = Color3.new(0, 0, 0),
 Round = 8,
  Z = Level + 3
 }), Library:Create("UIGradient", {, Parent = Items.Shade.Instance,, Rotation = 90,, Transparency = NumberSequence.new({, NumberSequenceKeypoint.new(0, 1),, 
 ...
 Pos = Pos,
  ...
  Raw = Color3.new(1, 1, 1),
 Alpha = 1,, Round = 20,, Z = Z, }), Library:Create("UIStroke", {, Parent = Cursor.Instance,
  Color = Color3.new(1, 1, 1),
 Thickness = CursorThickness, }), return Cursor, end, Items.FieldCursor = MakeCursor(, Items.Field.Instance,
 ...
 Parent = Items.AlphaBar.Instance,, Transparency = NumberSequence.new({, NumberSequenceKeypoint.new(0, 0),, NumberSequenceKeypoint.new(1, 1), }), }), Items.Hex =
  MakeFrame({, Parent = Items.Window.Instance,
 ...
 Clip = true,
  Z = Level + 1
 }), Items.HexInput = MakeInput({, Parent = Items.Hex.Instance,, Text = "#FFFFFF",, Placeholder = "#FFFFFF",
 ...
 if Silent then
  Picker.Color = Color3.fromHSV(Picker.Hue, Picker.Saturation, Picker.Value)
 return, end, Refresh(true), end, local function Slide(Input)
...
Picker:Set(Color, Picker.Transparency), else, Refresh(true), end, end), AttachPopup({, Popup = Picker,, Frame = Items.Window,, Level = Level,, GetAnchor = 
GetAnchor,, Place = PlaceBeside(GetAnchor), })
...
OnPicked(Input.KeyCode), else, OnPicked(Input.UserInputType), end, task.defer(function(), Library.Binding = false, end), end), end
...
Color = "Section",, Round = 10,, Z = 80, }), Items.Icon = MakeImage({, Parent = Items.Frame.Instance,, Icon = Icon,
 Pos = UDim2.fromOffset(13, 11),
 Size = UDim2.fromOffset(18, 18),
Raw = Accent,, Z = 81, }), Items.Title = MakeText({, Parent = Items.Frame.Instance,, Text = Title,, TextSize = 15,
 Pos = UDim2.fromOffset(40, 10),
 Size = UDim2.new(1, -70, 0, 20),
Color = "Text",, Truncate = true,, Z = 81, }), Items.Close = MakeImage({, Parent = Items.Frame.Instance,, Icon = "x",
 Anchor = Vector2.new(1, 0),
 Pos = UDim2.new(1, -13, 0, 13),
 Size = UDim2.fromOffset(14, 14),
Color = "DimText",, Z = 82, }), Items.CloseHit = MakeButton({, Parent = Items.Frame.Instance,
 Anchor = Vector2.new(1, 0),
...
Color = "DimText",, Wrap = true,, Z = 81, }), Items.Body.Instance.TextYAlignment = Enum.TextYAlignment.Top, end, Items.BarBack = MakeFrame({, Parent = 
Items.Frame.Instance,
 Pos = UDim2.new(0, 13, 1, -12),
 Size = UDim2.new(1, -26, 0, 5),
Color = "Element",, Round = 4,, Z = 81, }), Items.BarFill = MakeFrame({, Parent = Items.BarBack.Instance,
 Size = UDim2.new(1, 0, 1, 0),
 Raw = Color3.new(1, 1, 1),
Round = 4,, Z = 82, }), Library:RegisterGradient(Library:Create("UIGradient", {, Parent = Items.BarFill.Instance, }).Instance)
 local Notif = {
Items = Items,, Dead = false,, Height = CardH, }, table.insert(Library.Notifs, Notif), local function StackHeight(Stop)
 local Y = 15
...
Items.Frame.Instance:Destroy(), Reflow(), end), end, Items.CloseHit:Connect("MouseButton1Down", Dismiss)
...
Color = "Background",, Round = 10,, Clip = true,, Z = 1, }), Items.Rail = MakeFrame({, Parent = Items.Root.Instance,
 Pos = UDim2.fromOffset(0, RailY),
 Size = UDim2.fromOffset(RailW, RailH),
Color = "Section",, Round = 10,, Z = 1, }), Items.SubBar = MakeFrame({, Parent = Items.Root.Instance,
 Pos = UDim2.fromOffset(SubX, SubY),
 Size = UDim2.fromOffset(SubW, SubH),
Color = "Section",, Round = 10,, Clip = true,, Z = 20, }), Items.TopBar = MakeFrame({, Parent = Items.Main.Instance,
 Size = UDim2.fromOffset(W, TopH),
Color = "Section",, Round = 10,, Z = 2, }), MakeFrame({, Parent = Items.TopBar.Instance,
 Pos = UDim2.fromOffset(0, TopH - 12),
 Size = UDim2.fromOffset(W, 12),
Color = "Section",, Z = 2, }), Items.TopLine = MakeFrame({, Parent = Items.Main.Instance,
 Pos = UDim2.fromOffset(0, 50),
 Size = UDim2.fromOffset(W, 1),
Color = "Element",, Z = 3, }), Items.HubIcon = MakeImage({, Parent = Items.TopBar.Instance,, Icon = Window.Icon,
 Pos = UDim2.fromOffset(10, 10),
 Size = UDim2.fromOffset(30, 30),
 Raw = Color3.new(1, 1, 1),
Fit = true,, Z = 3, }), Items.Search = MakeFrame({, Parent = Items.TopBar.Instance,
 Pos = UDim2.fromOffset(50, 11),
 Size = UDim2.fromOffset(280, 30),
Color = "Element",, Round = 5,, Z = 3, }), MakeImage({, Parent = Items.Search.Instance,, Icon = "search",
 Pos = UDim2.fromOffset(7, 7),
 Size = UDim2.fromOffset(16, 16),
Color = "DimText",, Z = 4, }), Items.SearchBox = MakeInput({, Parent = Items.Search.Instance,, Placeholder = "search",
 Pos = UDim2.fromOffset(30, -1),
 Size = UDim2.new(1, -38, 1, 0),
TextSize = 15,, Z = 4, }), Items.Username = MakeText({, Parent = Items.TopBar.Instance,, Text = LocalPlayer.DisplayName,, TextSize = 15,
 Anchor = Vector2.new(1, 0),
 Pos = UDim2.new(1, -56, 0, 8),
 Size = UDim2.fromOffset(240, 18),
Color = "Text",, Align = Enum.TextXAlignment.Right,, Truncate = true,, Z = 3, }), Items.Version = MakeFrame({, Parent = Items.TopBar.Instance,
 Anchor = Vector2.new(1, 0),
 Pos = UDim2.new(1, -56, 0, 29),
 Size = UDim2.fromOffset(36, 14),
Color = "Background",, Round = 3,, Z = 3, }), Library:Create("UIStroke", {, Parent = Items.Version.Instance,, Color = Library.Theme.Element,, Thickness = 1
...
BackgroundColor3 = Library.Theme.Element,, ZIndex = Props.Z,, BorderSizePixel = 0,, Image = "rbxthumb://type=AvatarHeadShot&id=", .. LocalPlayer.UserId
...
Size = 30,, Res = 60,, Round = 20,, Z = 3, }), Items.ProfileHit = MakeButton({, Parent = Items.TopBar.Instance,
 Anchor = Vector2.new(1, 0),
...
Color = "Section",, Round = 10,, Z = 45, }), ProfilePanel.Instance.Visible = false, MakeAvatar(ProfilePanel.Instance, {
 Pos = UDim2.fromOffset(14, 14),
Size = 42,, Res = 100,, Round = 21,, Z = 46, }), MakeText({, Parent = ProfilePanel.Instance,, Text = LocalPlayer.DisplayName,, TextSize = 15,
 Pos = UDim2.fromOffset(68, 16),
 Size = UDim2.fromOffset(ProfileW - 82, 20),
Color = "Text",, Truncate = true,, Z = 46, }), MakeText({, Parent = ProfilePanel.Instance,
 Text = "@" .. LocalPlayer.Name,
TextSize = 13,
 Pos = UDim2.fromOffset(68, 37),
 Size = UDim2.fromOffset(ProfileW - 82, 16),
Color = "DimText",, Truncate = true,, Z = 46, }), local function ProfileDivider(Y), MakeFrame({, Parent = ProfilePanel.Instance,
...
Z = 47, }), IdHit:Connect("MouseButton1Down", function(), if setclipboard then, pcall(setclipboard, tostring(LocalPlayer.UserId)), end, Library:Notification({
 Name = "User ID copied",
 Description = "Your user id is now in the clipboard.",
Icon = "copy",, Duration = 3, }), end), local RefreshProfilePos, ProfileDivider(134), MakeText({, Parent = ProfilePanel.Instance,
...
Parent = ScaleTrack.Instance,
 ...
 Raw = Color3.new(1, 1, 1),
Round = 20,, Z = 47, }), Library:RegisterGradient(Library:Create("UIGradient", {, Parent = ScaleFill.Instance, }).Instance)
...
ApplyScale(Input), end, end), MakeText({, Parent = ProfilePanel.Instance,
 Text = "Menu toggle",
TextSize = 14,
 Anchor = Vector2.new(0, 0.5),
...
Color = "Section",, Round = 8,, Z = 46, }), KeyPanel.Instance.Visible = false, MakeText({, Parent = KeyPanel.Instance,
 Text = "Menu keybind",
TextSize = 12,
...
Color = "Light",, Round = 5,, Clip = true,, Z = 47, })
 local KeyText = MakeText({
Parent = KeyBox.Instance,
Text = KeyName(Library.MenuKeybind),
TextSize = 13,
 Size = UDim2.new(1, 0, 1, 0),
Color = "Text",, Align = Enum.TextXAlignment.Center,, Truncate = true,, Z = 48, })
 local KeyBoxHit = MakeButton({
Parent = KeyBox.Instance,
Z = 49
})
 local KeyState = {
IsOpen = false,, Debounce = false,, Picking = false,, Host = Profile, }, local function KeyPlace(Off)
...
return UDim2.fromOffset(PX, PY), end, AttachPopup({, Popup = KeyState,, Frame = KeyPanel,, Level = 46,, GetAnchor = function(), return KeyIconHit.Instance, 
end,, Place = KeyPlace,, From = -6,, To = 2,, Retreat = RetreatLeft, }), KeyIconHit:Connect("MouseButton1Down", function(), KeyState:SetOpen(not 
KeyState.IsOpen), end), KeyBoxHit:Connect("MouseButton1Click", function()
...
Library.MenuKeybind = Input.KeyCode, end, KeyText.Instance.Text = KeyName(Library.MenuKeybind), KeyState.Picking = false, task.defer(function(), Library.Binding
 = false, end), end), end)
 local UnloadButton = MakeFrame({
Parent = ProfilePanel.Instance,
 Pos = UDim2.fromOffset(14, 236),
 Size = UDim2.fromOffset(ProfileW - 28, 28),
Color = "Light",, Round = 6,, Clip = true,, Z = 46, }), MakeText({, Parent = UnloadButton.Instance,, Text = "Unload",, TextSize = 14,
 Size = UDim2.new(1, 0, 1, 0),
Color = "Text",
Align = Enum.TextXAlignment.Center,
Z = 47
...
end, RefreshProfilePos = function(), if Profile.IsOpen then, ProfilePanel.Instance.Position = ProfilePlace(10), end, end, AttachPopup({, Popup = Profile,, Frame
 = ProfilePanel,, Level = 45,, GetAnchor = function(), return Items.ProfileHit.Instance, end,, Place = ProfilePlace,, From = -2,, To = 10,, HoldOpen = 
function(), return KeyState.IsOpen, end, }), Window.Profile = Profile, Items.ProfileHit:Connect("MouseButton1Down", function(), Profile:SetOpen(not 
Profile.IsOpen), end), Items.Content = MakeFrame({, Parent = Items.Main.Instance,
 Pos = UDim2.fromOffset(15, 65),
 Size = UDim2.fromOffset(W - 30, H - 96),
Clip = true,, Z = 2, }), Window.Items = Items, Items.Root:MakeDraggable(Items.Main.Instance), Items.Root:MakeDraggable(Items.Rail.Instance), 
table.insert(Library.Windows, Window), function Window:Center()
...
end, function Window:LayoutSubBar(Instant), Window:FitSubBar(Instant), end, function Window:SetOpen(Bool), Window.IsOpen = Bool
 if not Bool then
...
Window:PlayIntro(), else, Items.Root:FadeDescendants(Bool), end, end, Library:Connect(UserInputService.InputBegan, function(Input, Processed)
 if Processed or Library.Binding then return end
 if Input.KeyCode == Library.MenuKeybind then
Window:SetOpen(not Window.IsOpen), end, end), local function RunSearch(Query), Query = string.lower(Query)
...
end, Data.Visible = Match, if Data.Section then, Data.Section.Dirty = true, end, end
 for _, Tab in Window.Tabs do
 for _, Sub in Tab.Subs do
 for _, Section in Sub.Sections do
if Section.Dirty then, Section.Dirty = false, Section:Reflow(), end, end, end, end
 local Tab = Window.Current
...
Window.SearchSig = Sig, Sub:Show(), end, end, end
 local SearchToken = 0
...
Items.Root.Instance.Position = UDim2.fromOffset(, Base.X.Offset,, Base.Y.Offset + 24, ), Items.Root:FadeDescendants(true)
 Library:Tween({ Position = Base }, Rise, Items.Root.Instance)
end, task.defer(function(), Window:PlayIntro(), end), return setmetatable(Window, Library), end
...
Color = "Element",, Round = 8,, Clip = true,, Z = 3, }), SetRest(Items.Row.Instance, "BackgroundTransparency", 1), Items.Bar = MakeFrame({, Parent = 
Window.Items.Rail.Instance,
 Anchor = Vector2.new(0, 0.5),
 Pos = UDim2.new(0, 0, 0, RowY + 20),
 Size = UDim2.fromOffset(3, 0),
Color = "Accent",, Round = 4,, Z = 4, }), Items.BarShadow = MakeAccentShadow(, Items.Bar.Instance,, UDim2.fromOffset(3, 15),, UDim.new(0, 10),, 0, ), if 
Items.BarShadow then, SetRest(Items.BarShadow, "Transparency", 1), end, Items.Icon = MakeImage({, Parent = Items.Row.Instance,, Icon = Tab.Icon,
 Pos = UDim2.fromOffset(10, 10),
 Size = UDim2.fromOffset(20, 20),
Color = "DimIcon",, Z = 4, }), Items.Hit = MakeButton({, Parent = Items.Row.Instance,, Z = 6, })
 Items.SubRow = Library:Create("ScrollingFrame", {
Parent = Window.Items.SubBar.Instance,, Name = "\0",, BackgroundTransparency = 1,, ScrollBarThickness = 0,, ScrollBarImageColor3 = Library.Theme.Accent,, 
ScrollingDirection = Enum.ScrollingDirection.X,, ScrollingEnabled = false,, Selectable = false,, Active = true,, AutomaticCanvasSize = Enum.AutomaticSize.X,
 CanvasSize = UDim2.fromOffset(0, 0),
 Size = UDim2.new(1, 0, 1, 0),
ZIndex = 21,
BorderSizePixel = 0
...
Parent = Items.SubRow.Instance,, FillDirection = Enum.FillDirection.Horizontal,, HorizontalAlignment = Enum.HorizontalAlignment.Left,, VerticalAlignment = 
Enum.VerticalAlignment.Center,, SortOrder = Enum.SortOrder.LayoutOrder,
...
Window:FitSubBar(true), end, end), Tab.Items = Items, function Tab:SetVisual(Active), Tab.Active = Active
...
Other:SetVisual(Other == Sub), end, Sub:Show(), end, function Tab:Select()
 if Window.Current == Tab then return end
Library:CloseAllPopups(), if Window.Current then, Window.Current:SetVisual(false), Window.Current.Items.SubRow.Instance.Visible = false, if 
Window.Current.Current then, Window.Current.Current:Hide(), end, end, Window.Current = Tab, Tab:SetVisual(true), Items.SubRow.Instance.Visible = true, 
EnterFirstSub(), Window:LayoutSubBar(), end, Items.Hit:Connect("MouseButton1Down", function(), Tab:Select(), end), table.insert(Window.Tabs, Tab), 
Window:LayoutRail()
 if #Window.Tabs == 1 then
Window.Current = Tab, Tab:SetVisual(true), Items.SubRow.Instance.Visible = true, task.defer(function(), EnterFirstSub(), Window:LayoutSubBar(), end), end, 
return setmetatable(Tab, Library), end
...
Color = "Element",, Round = 5,, Clip = true,, Z = 22, }), SetRest(Items.Pill.Instance, "BackgroundTransparency", 1), Items.Pill.Instance.LayoutOrder = 
#Tab.Subs, Items.Icon = MakeImage({, Parent = Items.Pill.Instance,, Icon = SubTab.Icon,
 Anchor = Vector2.new(0, 0.5),
 Pos = UDim2.new(0, 11, 0.5, 0),
 Size = UDim2.fromOffset(18, 18),
Color = "DimIcon",, Z = 23, }), Items.Label = MakeText({, Parent = Items.Pill.Instance,, Text = SubTab.Name,, TextSize = 15,
 Anchor = Vector2.new(0, 0.5),
 Pos = UDim2.new(0, 37, 0.5, 0),
 Size = UDim2.fromOffset(TextW + 8, 20),
Color = "Text",, Z = 23, }), SetRest(Items.Label.Instance, "TextTransparency", 1), local function SyncWidth()
...
Window:FitSubBar(true), end, end, Library:Connect(Items.Label.Instance:GetPropertyChangedSignal("TextBounds"), SyncWidth), task.defer(SyncWidth), Items.Hit = 
MakeButton({, Parent = Items.Pill.Instance,, Z = 25, }), Items.Page = MakeFrame({, Parent = Library.UnusedHolder.Instance,
...
Parent = Items.Page.Instance,, Name = "\0",, BackgroundTransparency = 1,, ScrollBarThickness = 0,, ScrollBarImageTransparency = 1,, Selectable = false,, Active 
= true,
...
end, return Column, end, SubTab.Columns[1] = MakeColumn(0), SubTab.Columns[2] = MakeColumn(Col2X), SubTab.Items = Items, function SubTab:SetVisual(Active, 
Instant)
...
Items.Pill:OnHover(function()
 ...
 local Objects = {
Parts.Header.Instance,, Parts.HeaderFill.Instance,, Parts.Label.Instance,, Parts.Frame.Instance,, Parts.BodyFill.Instance, }
 for _, Object in Objects do
 local Properties = Library:GetTweenProperty(Object)
 if not Properties then continue end
 for _, Property in Properties do
Handler(Object, Property), end, end, end, local function PrimeParts(Section), ForEachSectionPart(Section, function(Object, Property), 
Library:CaptureResting(Object, Property), Object[Property] = 1, end), end, local function RevealParts(Section), ForEachSectionPart(Section, function(Object, 
Property)
 local Resting = Library:CaptureResting(Object, Property)
...
local function ForEachRow(Handler)
 ...
 for _, Section in Column.Sections do
 for _, Data in Section.Rows do
Handler(Data, Section), end, end, end, end, local PageTween, local function StopTween(Handle)
 if not Handle then return end
pcall(function(), Handle:Cancel(), end), end, local function StopPageTween(), StopTween(PageTween), PageTween = nil, end, local function StopRowTweens(), 
ForEachRow(function(Data), StopTween(Data.Move), Data.Move = nil, end), end, local function LayoutPage()
 for _, Column in SubTab.Columns do
 for _, Section in Column.Sections do
...
end, end, end, end, local function CleanPage()
...
SubTab.Active = true, StopPageTween(), StopRowTweens(), Items.Page:CancelFade(), Items.Page:HardRestore(), Items.Page.Instance.Parent = 
Window.Items.Content.Instance
 Items.Page.Instance.Position = UDim2.fromOffset(0, 0)
...
Data.Frame:FadeDescendants(true), end), end, end), end, end, end, function SubTab:Hide(OnDone), SubTab.Active = false, SubTab.ShowToken += 1, 
Library:SafeCall(SubTab.PageOutro), StopRowTweens(), ForEachRow(function(Data), Data.Frame:CancelFade(), end), StopPageTween(), PageTween = Library:Tween({
...
StopPageTween(), Items.Page:ResetFade(), CleanPage(), Items.Page.Instance.Visible = false, Items.Page.Instance.Parent = Library.UnusedHolder.Instance, end
 if OnDone then Library:SafeCall(OnDone) end
end), end, function SubTab:SnapVisible(), SubTab.ShowToken += 1, SubTab.Active = true, StopPageTween(), StopRowTweens(), Items.Page:CancelFade(), CleanPage(), 
Items.Page.Instance.Visible = true, end, Items.Hit:Connect("MouseButton1Down", function()
LayoutPage()
ForEachRow(function(Data)
 Data.Frame.Instance.Position = UDim2.fromOffset(0, Data.Y)
end)
...
Items.Page.Instance.Visible = true, end, Items.Hit:Connect("MouseButton1Down", function()
 if Tab.Current == SubTab then return end
Library:CloseAllPopups(), if Tab.Current then, Tab.Current:SetVisual(false), Tab.Current:Hide(), end, Tab.Current = SubTab, SubTab:SetVisual(true), 
SubTab:Show(), Window:LayoutSubBar(), end), table.insert(Tab.Subs, SubTab)
 if #Tab.Subs == 1 then
Tab.Current = SubTab
...
SubTab = SubTab,, Column = Column,, Width = Column.Width,, Y = 0,, Height = 0,
 Rows = { },
Dirty = false,
 Items = { }
}
...
Color = "Section",, Round = 10,, Z = 3, }), Items.HeaderFill = MakeFrame({, Parent = Items.Header.Instance,
 Pos = UDim2.fromOffset(0, 15),
 Size = UDim2.fromOffset(HeaderW, 10),
Color = "Section",, Z = 3, }), Items.Label = MakeText({, Parent = Items.Header.Instance,, Text = Section.Name,, TextSize = 15,
 Anchor = Vector2.new(0, 0.5),
...
end, Library:Connect(Items.Label.Instance:GetPropertyChangedSignal("TextBounds"), SyncHeader), task.defer(SyncHeader), Items.Frame = MakeFrame({, Parent = 
Items.Holder.Instance,
 Pos = UDim2.fromOffset(0, 26),
 Size = UDim2.fromOffset(Column.Width, 14),
Color = "Section",, Round = 10,, Z = 3, }), Items.BodyFill = MakeFrame({, Parent = Items.Frame.Instance,
 Pos = UDim2.fromOffset(0, 0),
 Size = UDim2.fromOffset(10, 10),
Color = "Section",, Z = 3, }), Section.Items = Items, function Section:Reflow()
 local Y = 8
 local Visible = 0
 for _, Data in Section.Rows do
...
Section = Section,, Window = SubTab.Window, }, table.insert(Section.Rows, Data), table.insert(Library.Searchables, Data), Section:Reflow(), return Frame, Data, 
end, table.insert(SubTab.Sections, Section), table.insert(Column.Sections, Section), Section:Reflow(), return setmetatable(Section, Library), end
...
Color = "DimText",, Truncate = true,, Z = 5, }), Items.Box = MakeFrame({, Parent = Row.Instance,
 Anchor = Vector2.new(1, 0.5),
 Pos = UDim2.new(1, -15, 0.5, 0),
 Size = UDim2.fromOffset(35, 21),
Color = "Element",, Round = 10,, Clip = true,, Z = 5, }), Items.Circle = MakeFrame({, Parent = Items.Box.Instance,
 Anchor = Vector2.new(0, 0.5),
 Pos = UDim2.new(0, 4, 0.5, 0),
 Size = UDim2.fromOffset(13, 13),
Color = "DimText",, Round = 20,, Z = 6, }), MakeAccentShadow(, Items.Circle.Instance,, UDim2.fromOffset(3, 3),, UDim.new(0, 5),, 0.7, ), Items.Hit = 
MakeButton({, Parent = Row.Instance,
 Size = UDim2.new(1, 0, 1, 0),
Z = 8
})
Toggle.Items = Items
...
if Toggle.GrowTween then, pcall(function(), Toggle.GrowTween:Cancel(), end), end, if Toggle.SnapTween then, pcall(function(), Toggle.SnapTween:Cancel(), end), 
end, Toggle.GrowTween = nil, Toggle.SnapTween = nil
...
if Instant then, Items.Box.Instance.BackgroundColor3 = Library.Theme[BoxColor], Items.Label.Instance.TextColor3 = Library.Theme[LabelColor], 
Items.Circle.Instance.BackgroundColor3 = CircleColor, Items.Circle.Instance.Size = Small, Items.Circle.Instance.AnchorPoint = EndAnchor, 
Items.Circle.Instance.Position = EndPos, return, end
...
Items.Circle.Instance.AnchorPoint = EndAnchor, Items.Circle.Instance.Position = EndPos, Items.Circle.Instance.Size = Small, end), end, function Toggle:Set(Bool)
 Toggle.Value = Bool and true or false
if Toggle.Flag then, Library.Flags[Toggle.Flag] = Toggle.Value, end, Toggle.SetVisual(Toggle.Value, false), Library:SafeCall(Toggle.Callback, Toggle.Value), 
end, function Toggle:Get(), return Toggle.Value, end, Items.Hit:Connect("MouseButton1Down", function(), Toggle:Set(not Toggle.Value), end), Toggle.SlotX = -58, 
...
end), return Glyph, Hit, end, local function SidePlace(Anchor), return function(Off)
 local PScale = Library:GetScreenScale()
 local Right = Anchor.AbsolutePosition.X + Anchor.AbsoluteSize.X
 local PX = Right / PScale + 8 + (Off or 0)
 local PY = (Anchor.AbsolutePosition.Y + GuiInset) / PScale - 4
...
Picker.Color = Color, Picker.Transparency = Alpha, Swatch:SetColor(Color, Alpha), if CParams.Flag then, Library.Flags[CParams.Flag] = {, __color = 
Color:ToHex(),, __alpha = Alpha, }, end, Library:SafeCall(CParams.Callback, Color, Alpha), end), Swatch.Hit:Connect("MouseButton1Down", function(), 
Inner:SetOpen(not Inner.IsOpen), end), if CParams.Flag then
 Library.SetFlags[CParams.Flag] = function(Color, Alpha)
Inner:Set(Color, Alpha), end, end, function Picker:Set(Color, Alpha), Inner:Set(Color, Alpha), end, Picker.Picker = Inner, return Picker, end, function 
Toggle:Keybind(KParams)
 KParams = KParams or { }
 local Keybind = {
Key = KParams.Default,
 Mode = KParams.Mode or "Toggle",
Flag = KParams.Flag,, Picking = false,, IsOpen = false,, Debounce = false, }
 local X = TakeSlot(20)
...
Color = "Section",, Round = 8,, Z = 40, }), Panel.Instance.Visible = false, MakeText({, Parent = Panel.Instance,, Text = "Keybind",, TextSize = 12,
...
TextSize = 13,
 Size = UDim2.new(1, 0, 1, 0),
Color = "Text",, Align = Enum.TextXAlignment.Center,, Truncate = true,, Z = 42, })
...
LineX = 6,, LineH = 14,, TextX = 10,, TextActiveX = 17,, SnapShadow = true,, Z = 41, }), Built.Hit:Connect("MouseButton1Down", function(), 
Keybind:SetMode(ModeName), end), table.insert(ModeRows, {, Name = ModeName,, SetActive = Built.SetActive, }), end, ModeRow(60, "Toggle"), ModeRow(92, "Hold"), 
local function SaveFlag()
 if not Keybind.Flag then return end
Library.Flags[Keybind.Flag] = {
 Key = Keybind.Key and tostring(Keybind.Key) or "None",
Mode = Keybind.Mode, }, end, function Keybind:SetMode(Mode, Instant), Keybind.Mode = Mode
 for _, Data in ModeRows do
 Data.SetActive(Data.Name == Mode, Instant)
end, SaveFlag(), end, function Keybind:Set(Key), Keybind.Key = Key, PanelKey.Instance.Text = KeyName(Key), Keybind.Picking = false, SaveFlag(), end, 
AttachPopup({, Popup = Keybind,, Frame = Panel,, Level = 40,, GetAnchor = function(), return BindHit.Instance, end,, Place = SidePlace(BindIcon.Instance),, From
 = -6,, To = 2,, Retreat = RetreatLeft, }), BindHit:Connect("MouseButton1Down", function(), Keybind:SetOpen(not Keybind.IsOpen), end), 
KeyHit:Connect("MouseButton1Click", function(), CaptureKey(Keybind, PanelKey.Instance, function(Key), Keybind:Set(Key), end), end), 
...
Toggle:Set(true), else, Toggle:Set(not Toggle.Value), end, end), Library:Connect(UserInputService.InputEnded, function(Input)
...
Value = Value.Key, end, Keybind:Set(ParseKey(Value)), end, end, Keybind:SetMode(Keybind.Mode, true), Keybind:Set(Keybind.Key), return Keybind, end, function 
Toggle:Extra(EParams)
 EParams = EParams or { }
...
Color = "Section",, Round = 8,, Z = 2, }), Frame.Instance.Visible = false, MakeText({, Parent = Frame.Instance,, Text = Toggle.Name,, TextSize = 12,
...
Parent = Frame.Instance,
 Size = UDim2.new(1, 0, 1, 0),
 Raw = Color3.new(0, 0, 0),
Alpha = 1,, Round = 8,, Z = 30, }), ChildDim.Instance.Visible = false, Library:StampResting(ChildDim.Instance, "BackgroundTransparency", 1)
...
end, return false, end, AttachPopup({, Popup = Extra,, Frame = Frame,, Level = 2,, GetAnchor = function(), return ExtraHit.Instance, end,, Place = 
SidePlace(ExtraIcon.Instance),, From = -6,, To = 2,, Retreat = RetreatLeft,, KeepOpen = function(Value)
...
end, end, }), ExtraHit:Connect("MouseButton1Down", function(), Extra:SetOpen(not Extra.IsOpen), end), setmetatable(Extra, {
...
end, return Element, end, end, }), Toggle.ExtraPanel = Extra, return Extra, end, Toggle.Value = Toggle.Default, if Toggle.Flag then, Library.Flags[Toggle.Flag] 
= Toggle.Value, Library.SetFlags[Toggle.Flag] = function(Value), Toggle:Set(Value), end, end, Toggle.SetVisual(Toggle.Value, true), 
Library:SafeCall(Toggle.Callback, Toggle.Value), return setmetatable(Toggle, Library), end
...
Color = "Text",, Truncate = true,, Z = 5, }), Items.Value = MakeText({, Parent = Row.Instance,, Text = "",, TextSize = 15,
 Anchor = Vector2.new(1, 0),
 Pos = UDim2.new(1, -15, 0, 3),
 Size = UDim2.fromOffset(100, 20),
Color = "DimText",, Align = Enum.TextXAlignment.Right,, Truncate = true,, Z = 5, }), Items.Track = MakeFrame({, Parent = Row.Instance,
 Anchor = Vector2.new(0, 0.5),
 Pos = UDim2.new(0, 15, 0, 32),
 Size = UDim2.fromOffset(TrackW, 10),
Color = "Element",, Round = 20,, Z = 5, }), Items.Fill = MakeFrame({, Parent = Items.Track.Instance,
 Size = UDim2.fromOffset(0, 10),
 Raw = Color3.new(1, 1, 1),
Round = 20,, Z = 6, }), Library:RegisterGradient(Library:Create("UIGradient", {, Parent = Items.Fill.Instance, }).Instance), Items.Hit = MakeButton({, Parent = 
Row.Instance,
 Pos = UDim2.fromOffset(9, 22),
 Size = UDim2.fromOffset(TrackW + 12, 22),
Z = 8, }), Items.TrackWidth = TrackW, return Items, end
...
Library:SafeCall(Slider.Callback, Slider.Value), end, function Slider:Get(), return Slider.Value, end, local function Calculate(Input)
local Fraction = AxisFraction(Input, Items.Track.Instance, "X")
 return Slider.Min + (Slider.Max - Slider.Min) * Fraction
end, local function Apply(Input), Slider:Set(Calculate(Input)), end, AttachDrag(Items.Hit, {, OnGrab = function(Input), Slider.Sliding = true, Apply(Input), 
end,, OnMove = Apply,, OnRelease = function(), Slider.Sliding = false, end, }), Slider:Set(Slider.Default, true), if Slider.Flag then, 
Library.SetFlags[Slider.Flag] = function(Value), Slider:Set(Value), end, end, return setmetatable(Slider, Library), end
...
Items.Value.Instance.Text = Text, Library:SafeCall(Slider.Callback, Slider.Value), end, function Slider:Get(), return Slider.Value, end, local function 
Apply(Input)
...
Apply(Input), end,, OnMove = Apply,, OnRelease = function(), Slider.Grabbing = nil, end, }), Slider:Set(Slider.Default[1], Slider.Default[2], true), if 
Slider.Flag then, Library.SetFlags[Slider.Flag] = function(Value), Slider:Set(Value), end, end, return setmetatable(Slider, Library), end
...
Color = "Element",, Round = 6,, Clip = true,, Z = 5, }), return Items, end
...
Color = "Accent",, Round = 6,, Z = Z, }), Sweep.Instance.BackgroundTransparency = 1, Library:StampResting(Sweep.Instance, "BackgroundTransparency", 1), return 
Sweep, end, local function PlaySweep(Sweep)
...
Library:Tween({
 Size = UDim2.new(0, 0, 1, 0),
BackgroundTransparency = 1, }, Out, Sweep), end), end, local function HoverSwap(Frame), Frame:OnHover(function()
...
Color = "Text",, Truncate = true,, Z = 6, }), Items.Arrow = MakeImage({, Parent = Items.Box.Instance,, Icon = "chevron-down",
 Anchor = Vector2.new(1, 0.5),
 Pos = UDim2.new(1, -10, 0.5, 0),
 Size = UDim2.fromOffset(14, 14),
Color = "DimText",, Z = 6, }), Items.Hit = MakeButton({, Parent = Items.Box.Instance,, Z = 7, }), Dropdown.Items = Items
...
end, Dropdown.Popup = Popup, local function Report(), if Dropdown.Flag then, Library.Flags[Dropdown.Flag] = Dropdown.Value, end, if Dropdown.Multi then
...
and tostring(Dropdown.Value), or "None", end, Library:SafeCall(Dropdown.Callback, Dropdown.Value), end, Popup.OnPick = function(Data), if Dropdown.Multi then
 local Index = table.find(Dropdown.Value, Data.Name)
if Index then, table.remove(Dropdown.Value, Index), Data:Set(false), else, table.insert(Dropdown.Value, Data.Name), Data:Set(true), end, else, Dropdown.Value = 
Data.Name
 for _, Other in Popup.Order do
Other:Set(Other == Data), end, end, Report(), end, function Dropdown:Refresh(List), Popup:Clear(), Dropdown.Options = List
 for _, Option in List do
Popup:AddRow(tostring(Option)), end, end, function Dropdown:Set(Value), if Dropdown.Multi then
...
end, end, Report(), end, function Dropdown:Get(), return Dropdown.Value, end, Items.Hit:Connect("MouseButton1Down", function(), Popup:SetOpen(not Popup.IsOpen),
 end), HoverSwap(Items.Box)
 for _, Option in Dropdown.Options do
Popup:AddRow(tostring(Option))
end
 if Dropdown.Default ~= nil then
Dropdown:Set(Dropdown.Default), end, if Dropdown.Flag then, Library.SetFlags[Dropdown.Flag] = function(Value), Dropdown:Set(Value), end, end, return 
setmetatable(Dropdown, Library), end
...
Parent = Row.Instance,
 Pos = UDim2.fromOffset(15, 3),
 Size = UDim2.fromOffset(Section.Width - 30, 30),
Color = "Element",, Round = 6,, Clip = true,, Z = 5, })
...
TextSize = 15,
 Size = UDim2.new(1, 0, 1, 0),
Color = "Text",, Align = Enum.TextXAlignment.Center,, Truncate = true,, Z = 7, }), Items.Hit = MakeButton({, Parent = Items.Frame.Instance,, Z = 8, }), 
Button.Items = Items, HoverSwap(Items.Frame), function Button:Press(), PlaySweep(Items.Sweep.Instance), Library:SafeCall(Button.Callback), end, function 
Button:SetText(Text), Items.Label.Instance.Text = tostring(Text), end, Items.Hit:Connect("MouseButton1Down", function(), Button:Press(), end), return 
setmetatable(Button, Library), end
...
TextSize = 15,, Z = 6, }), Textbox.Items = Items, function Textbox:Set(Value), Textbox.Value = tostring(Value), Items.Input.Instance.Text = Textbox.Value, if 
Textbox.Flag then, Library.Flags[Textbox.Flag] = Textbox.Value, end, Library:SafeCall(Textbox.Callback, Textbox.Value), end, function Textbox:Get(), return 
Textbox.Value, end, if Textbox.Finished then, Items.Input:Connect("FocusLost", function(Enter), if Enter then, Textbox:Set(Items.Input.Instance.Text), end, 
end), else, Library:Connect(Items.Input.Instance:GetPropertyChangedSignal("Text"), function(), Textbox:Set(Items.Input.Instance.Text), end), end
 if Textbox.Default ~= "" then
Textbox:Set(Textbox.Default), end, if Textbox.Flag then, Library.SetFlags[Textbox.Flag] = function(Value), Textbox:Set(Value), end, end, return 
setmetatable(Textbox, Library), end
...
Color = "Text",, Truncate = true,, Z = 5, }), Items.Icon = MakeImage({, Parent = Row.Instance,, Icon = "command",
 Anchor = Vector2.new(1, 0.5),
 Pos = UDim2.new(1, -16, 0.5, 0),
 Size = UDim2.fromOffset(16, 16),
Color = "DimText",, Z = 6, }), Items.Hit = MakeButton({, Parent = Row.Instance,
 Anchor = Vector2.new(1, 0.5),
...
Color = "Section",, Round = 8,, Z = 40, }), Panel.Instance.Visible = false, MakeText({, Parent = Panel.Instance,, Text = Keybind.Name,, TextSize = 12,
 Pos = UDim2.fromOffset(11, 8),
 Size = UDim2.fromOffset(PanelW - 22, 14),
Color = "DimText",
Truncate = true,
...
Color = "Light",, Round = 5,, Clip = true,, Z = 41, })
 local PanelKey = MakeText({
Parent = KeyBox.Instance,
Text = "None",
TextSize = 13,
...
Color = "Text",, Align = Enum.TextXAlignment.Center,, Truncate = true,, Z = 42, })
 local KeyHit = MakeButton({
Parent = KeyBox.Instance,, Z = 43, }), function Keybind:Set(Key), Keybind.Key = Key, PanelKey.Instance.Text = KeyName(Key), Keybind.Picking = false, if 
Keybind.Flag then
 Library.Flags[Keybind.Flag] = Key and tostring(Key) or "None"
end, end, function Keybind:Get(), return Keybind.Key, end, AttachPopup({, Popup = Keybind,, Frame = Panel,, Level = 40,, GetAnchor = function(), return 
Items.Hit.Instance, end,, Place = function(Off)
 local Anchor = Items.Icon.Instance
 local PScale = Library:GetScreenScale()
 local Right = Anchor.AbsolutePosition.X + Anchor.AbsoluteSize.X
 local PX = Right / PScale + 8 + (Off or 0)
 local PY = (Anchor.AbsolutePosition.Y + GuiInset) / PScale - 4
return UDim2.fromOffset(PX, PY), end,, From = -6,, To = 2,, Retreat = RetreatLeft, }), Items.Hit:Connect("MouseButton1Down", function(), Keybind:SetOpen(not 
Keybind.IsOpen), end), KeyHit:Connect("MouseButton1Click", function(), CaptureKey(Keybind, PanelKey.Instance, function(Key), Keybind:Set(Key), end), end), 
Library:Connect(UserInputService.InputBegan, function(Input, Processed)
 if Processed or Keybind.Picking or not Keybind.Key then return end
 if not KeyMatches(Input, Keybind.Key) then return end
Library:SafeCall(Keybind.Callback, Keybind.Key), end), Keybind:Set(Keybind.Key), if Keybind.Flag then, Library.SetFlags[Keybind.Flag] = function(Value), 
Keybind:Set(ParseKey(Value)), end, end, return setmetatable(Keybind, Library), end
...
Colorpicker.Color = Color, Colorpicker.Transparency = Alpha, Items.Swatch:SetColor(Color, Alpha), if Colorpicker.Flag then, Library.Flags[Colorpicker.Flag] = {,
 __color = Color:ToHex(),, __alpha = Alpha, }, end, Library:SafeCall(Colorpicker.Callback, Color, Alpha), end), Colorpicker.Picker = Picker, function 
Colorpicker:Set(Color, Alpha), Picker:Set(Color, Alpha), end, function Colorpicker:Get(), return Colorpicker.Color, Colorpicker.Transparency, end, 
Items.Swatch.Hit:Connect("MouseButton1Down", function(), Picker:SetOpen(not Picker.IsOpen), end), if Colorpicker.Flag then
 Library.SetFlags[Colorpicker.Flag] = function(Color, Alpha)
Picker:Set(Color, Alpha), end, end, return setmetatable(Colorpicker, Library), end
...
Color = "DimText",, Truncate = true,, Z = 5, }), function Label:Set(Text), Label.Items.Text.Instance.Text = tostring(Text), end, return setmetatable(Label, 
Library), end
...
Color = Color,, Wrap = true,, Z = 5, }), Item.Instance.TextYAlignment = Enum.TextYAlignment.Top, return Item, end
...
function Paragraph:SetTitle(Text), Paragraph.Items.Title.Instance.Text = tostring(Text), end, function Paragraph:SetContent(Text), 
Paragraph.Items.Body.Instance.Text = tostring(Text), end, return setmetatable(Paragraph, Library), end
...
Color = "Element",, Round = 6,, Clip = true,, Z = 4, }), MakeImage({, Parent = Items.NameBox.Instance,, Icon = "pencil",
 Anchor = Vector2.new(0, 0.5),
 Pos = UDim2.new(0, 12, 0.5, 0),
 Size = UDim2.fromOffset(14, 14),
Color = "DimText",, Z = 5, }), Items.NameInput = MakeInput({, Parent = Items.NameBox.Instance,
 Placeholder = "config name",
 Pos = UDim2.fromOffset(34, 0),
 Size = UDim2.new(1, -44, 1, 0),
TextSize = 15,, Z = 5, }), Items.Create = MakeFrame({, Parent = Items.CreateBox.Instance,
 Anchor = Vector2.new(1, 0),
 Pos = UDim2.new(1, 0, 0, 0),
 Size = UDim2.fromOffset(84, 40),
Color = "Element",, Round = 6,, Clip = true,, Z = 4, }), HoverSwap(Items.Create)
...
TextSize = 15,
 Size = UDim2.new(1, 0, 1, 0),
Color = "Text",, Align = Enum.TextXAlignment.Center,, Z = 6, }), Items.CreateHit = MakeButton({, Parent = Items.Create.Instance,, Z = 7, }), Items.ListHolder = 
MakeFrame({, Parent = Page.Instance,
 Pos = UDim2.fromOffset(0, 52),
 Size = UDim2.fromOffset(ColW, ContentH - 52),
Z = 3
})
 Items.List = Library:Create("ScrollingFrame", {
Parent = Items.ListHolder.Instance,, Name = "",, BackgroundTransparency = 1,, ScrollBarThickness = 0,, ScrollBarImageTransparency = 1,, Selectable = false,, 
Active = true,
 Size = UDim2.new(1, 0, 1, 0),
 CanvasSize = UDim2.fromOffset(0, 0),
ZIndex = 3,, BorderSizePixel = 0, }), Library:Create("UIListLayout", {, Parent = Items.List.Instance,, SortOrder = Enum.SortOrder.LayoutOrder,
...
Color = "Section",, Round = 10,, Z = 3, }), MakeText({, Parent = Items.InfoPanel.Instance,
 Text = "Config info",
TextSize = 15,
...
Icon = Icon,
 ...
 Size = UDim2.fromOffset(14, 14),
Color = "DimIcon",, Z = 4, }), MakeText({, Parent = Items.InfoPanel.Instance,, Text = Label,, TextSize = 15,
...
Color = "Text",, Align = Enum.TextXAlignment.Right,, Truncate = true,, Z = 4, })
 if Index < 5 then
MakeFrame({
Parent = Items.InfoPanel.Instance,
 Pos = UDim2.fromOffset(14, Y + 27),
 Size = UDim2.fromOffset(ColW - 28, 1),
Color = "Element",, Z = 4, }), end, return Value, end
...
Count += 1
 ...
 Color = "Section",, Round = 10,, Z = 3, }), MakeText({, Parent = Items.ThemePanel.Instance,, Text = "Theme",, TextSize = 15,
 ...
 end, end, MakeText({, Parent = Items.ThemePanel.Instance,, Text = "Presets",, TextSize = 15,
 ...
 Parent = Items.ThemePanel.Instance,
  ...
  Size = UDim2.fromOffset(18, 18),
  Raw = Preset.Swatch or Preset.Accent or Library.Theme.Accent,
 Round = 20,
 Z = 4
 })
 ...
 Thickness = 0
  ...
  local Hit = MakeButton({
 Parent = Dot.Instance,, Z = 5, }), Hit:Connect("MouseButton1Down", function(), Library:SetTheme(Preset), SelectPreset(Dot), if RefreshThemeUI then, 
 RefreshThemeUI(), end, end), table.insert(PresetDots, Dot), end, if PresetDots[1] and PresetDots[1].Ring and PresetDots[1].Ring.Instance then PresetDots[1].Ring.Instance.Thickness = 2 end
  local ThemeCells = {
 ...
 Swatch:SetColor(Color), Library:SetThemeColor(Key, Color), end), Swatch.Hit:Connect("MouseButton1Down", function(), CellPicker:SetOpen(not CellPicker.IsOpen), 
 end), table.insert(ThemeCellList, {, Key = Key,, Swatch = Swatch,, Picker = CellPicker, }), end, MakeText({, Parent = Items.ThemePanel.Instance,, Text = 
 "Accent",, TextSize = 15,
 ...
 Items.Swatch:SetColor(Color), Library:SetAccent(Color), end), Items.Swatch.Hit:Connect("MouseButton1Down", function(), Picker:SetOpen(not Picker.IsOpen), end), 
 RefreshThemeUI = function(), Items.Swatch:SetColor(Library.Theme.Accent), Picker:Set(Library.Theme.Accent, 0, true)
  for _, Cell in ThemeCellList do
 Cell.Swatch:SetColor(Library.Theme[Cell.Key]), Cell.Picker:Set(Library.Theme[Cell.Key], 0, true), end, end, local RefreshList
 ...
 Bar.Instance,, UDim2.fromOffset(3, 15),, UDim.new(0, 10),, 0, ), if BarShadow then, SetRest(BarShadow, "Transparency", 1), end
 ...
 end), Hit:Connect("MouseButton1Down", Callback), end, IconButton(-66, "download", function(), local Created, pcall(function(), Created = 
 HttpService:JSONDecode(readfile(ConfigPath(Name))).__created, end), writefile(ConfigPath(Name), Library:GetConfig(Created)), ShowInfo(Name), 
 Library:Notification({
  Name = "Config saved",
  Description = "Current values were written into \"" .. Name .. "\".",
 Icon = "download", }), end), IconButton(-40, "share-2", function(), if setclipboard then, pcall(function(), setclipboard(readfile(ConfigPath(Name))), end), end,
  Library:Notification({
  Name = "Config copied",
 ...
 Other:SetSelected(Other == Data), end, ShowInfo(Name), Library:LoadConfigFile(Name), if RefreshThemeUI then, RefreshThemeUI(), end, Library:Notification({
  Name = "Config loaded",
  Description = "All values were restored from \"" .. Name .. "\".",
 Icon = "check", }), end), table.insert(Config.Rows, Data), return Data, end, RefreshList = function()
 ...
 Icon = "triangle-alert", }), return, end, writefile(ConfigPath(Name), Library:GetConfig()), Items.NameInput.Instance.Text = "", Config.Selected = Name, 
 RefreshList(), ShowInfo(Name), Library:Notification({
  Name = "Config created",
 ...
 end), end, end, SubTab.PageOutro = function(), Config.IntroToken += 1
  local Token = Config.IntroToken
 ...
 end), end, end, function Config:Refresh(), RefreshList(), end, RefreshList(), ShowInfo(nil), return Config, end
 ...
 Color = "Section",, Round = 8,, Z = 60, }), Items.Bar.Instance.AutomaticSize = Enum.AutomaticSize.X, Library:Create("UIPadding", {, Parent = Items.Bar.Instance,
  PaddingLeft = UDim.new(0, 12),
  PaddingRight = UDim.new(0, 12)
 }), Library:Create("UIListLayout", {, Parent = Items.Bar.Instance,, FillDirection = Enum.FillDirection.Horizontal,, VerticalAlignment = 
 Enum.VerticalAlignment.Center,, SortOrder = Enum.SortOrder.LayoutOrder,
  Padding = UDim.new(0, 10)
 }), local function NextOrder(), Order += 1, return Order, end, Items.Icon = MakeImage({, Parent = Items.Bar.Instance,, Icon = Icon,
  Size = UDim2.fromOffset(20, 20),
  Raw = Color3.new(1, 1, 1),
 Fit = true,, Z = 61, }), Items.Icon.Instance.LayoutOrder = NextOrder(), local function Separator()
  local Sep = MakeFrame({
 Parent = Items.Bar.Instance,
  Size = UDim2.fromOffset(1, 14),
 Color = "Light",, Z = 61, }), Sep.Instance.LayoutOrder = NextOrder(), end, local function Stat(Text)
  local Label = MakeText({
 Parent = Items.Bar.Instance,
 Text = Text,
 TextSize = 14,
  Size = UDim2.fromOffset(0, 16),
 Color = "DimText",, Z = 61, }), Label.Instance.AutomaticSize = Enum.AutomaticSize.X, Label.Instance.LayoutOrder = NextOrder(), return Label, end, Separator()
  local GameStat = Stat("...")
 Separator()
 ...
 return MarketplaceService:GetProductInfo(game.PlaceId)
  ...
  local Frames = 0
 Library:Connect(RunService.RenderStepped, function(), Frames += 1, end), Library:Thread(function(), while task.wait(0.5) do
 ...
 function Watermark:SetIcon(NewIcon), ApplyIcon(Items.Icon.Instance, NewIcon), end, function Watermark:SetName(), end, function Watermark:SetVisible(Bool), 
 Items.Bar:FadeDescendants(Bool), end, return Watermark, end
 ...
 SetFunction(Color3.fromHex(Value.__color), Value.__alpha), else, SetFunction(Value), end, end
 ...
 end, return Result, end, Library:Connect(RunService.Heartbeat, function(Delta), if Library.ThemeDirty then, Library.ThemeDirty = false, 
 Library:ApplyThemeInstant(), end
  if not Library.PreloadDirty then return end
  Library.PreloadClock += Delta or 0
  if Library.PreloadClock >= 0.35 then
 Library.PreloadDirty = false, Library.PreloadClock = 0, Library:PreloadAll(), end, end), getgenv().Zolar = Library, end, return Library
